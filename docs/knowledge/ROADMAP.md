...
| `apps/server` base            | 3h    | `SpringBootApplication`, `application.yml`, profiles `dev`/`test`/`prod`           |
| `libs/shared` base            | 3h    | `KuboBankException`, `ErrorResponse`, `DomainEvent`                                |
| Docker Compose                | 2h    | PostgreSQL + DBeaver, variables de entorno, volumen persistente                    |
...
| Tarea                      | Horas | Descripción                                                           |
| -------------------------- | ----- | --------------------------------------------------------------------- |
| Liquibase changelog master | 2h    | `db.changelog-master.yml` referenciando los scripts .sql              |
